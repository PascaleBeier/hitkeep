...
| 0C | In progress | Inventory filesystem operations and dependency-ordered `internal/` move manifest | The durable filesystem/package manifest records reviewed families, classifications, native exclusions, and rejected move candidates. `internal/assetstore` now has rooted symlink-containment proof; further Phase 9/10 waves remain frozen until every remaining direct child and operation has complete dependency/owner/build-tag/generated-file evidence |
