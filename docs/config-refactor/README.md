...
| 0C | In progress | Inventory filesystem operations and dependency-ordered `internal/` move manifest | The durable filesystem/package manifest records reviewed families, classifications, native exclusions, and rejected move candidates. `internal/assetstore` has rooted symlink-containment proof; `internal/blocking` has bounded/closed feed-response, same-directory staged cache replacement (not fsync/durability or cross-platform atomicity proof), serialized whole-update generations, and asynchronous-cancelled refresh proof; `internal/realtime` has bounded in-memory replay, shutdown-first broker close, and one-minute SSE deadline/reconnect proof. Further Phase 9/10 waves remain frozen until every remaining direct child and operation has complete dependency/owner/build-tag/generated-file evidence |
...
Push the reviewed CI/Helm/cache commits and prove the new exact head. Then execute the fail-closed private-doc attestation and authenticated Kind/Helm lifecycle in a real draft/tagged release with deployed token scopes before claiming Phases 6–7 complete. Produce a stabilization release, prove Darwin/Windows CGO artifacts and Homebrew/Scoop install, upgrade, and uninstall behavior, complete the operation/package inventories before another filesystem or `internal/` wave, and repeat the final Ponytail and spf13 Go review against the finished release graph.
