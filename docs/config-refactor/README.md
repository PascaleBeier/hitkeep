...
| 7 | In progress | Enforce issue #288 upgrade, recreation, and migration interruption gates | A mandatory tagged-release job seeds recognizable rows through the digest-pinned v2.12.0 image, migrates the exact candidate digest, verifies row/storage/ownership/graceful-shutdown invariants across two recreation cycles, and blocks final publication on failure; full-volume rollback plus Compose/Helm semantic gates remain pending |
| 8 | Completed | GoReleaser artifact parity and Linux build cutover; preserve future package-manager compatibility | Tagged Linux builds use pinned GoReleaser v2.18.0 and preserve all four legacy binary names; checksum/verification accepts the fixed release assets including `hitkeep.example.yaml`; Homebrew/Scoop publishing remains conditional on real Darwin/Windows CGO builds and install/upgrade/uninstall lifecycle smokes |
...
Implement quiescent full-volume rollback snapshot/restore acceptance, then Compose and Helm upgrade/recreation gates. Continue pure leaf move-only waves with import-path and affected-package proofs. Enable Homebrew tap and Scoop bucket publishing only after GoReleaser produces real Darwin/Windows CGO artifacts with stable names/checksums and platform lifecycle smokes prove install, upgrade, config discovery, service behavior, and uninstall.
