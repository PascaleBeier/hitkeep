...
| 3 | In progress | Production Cobra root with legacy-compatible routing | Factory-built Cobra root owns top-level execution and exact first-argument routing; full Cobra flag ownership, command contexts, and subprocess stream/exit parity remain pending |
| 4 | In progress | Switch production assembly to Viper with complete legacy parity | `config.Load` uses instance-based Viper through an OS-backed Afero boundary; catalog-driven normal-startup tests cover every active self-hosted env/flag/default/alias plus warning/redaction parity, while cloud/build-variant parity and stabilization remain pending |
...
| 7 | In progress | Enforce issue #288 upgrade, rollback, recreation, and migration interruption gates | The mandatory tagged-release smoke seeds through digest-pinned v2.12.0, migrates the exact candidate, verifies two recreation cycles, then restores a quiescent pre-migration snapshot into a fresh volume and proves legacy identities across restart; platform matrix, interruption prerequisite, and Compose/Helm semantic gates remain pending |
| 8 | In progress | GoReleaser release ownership while preserving future package-manager compatibility | Pinned GoReleaser v2.18.0 keeps snapshot raw Linux/cloud binaries for current consumers and now defines self-hosted tar.gz archives with LICENSE/README/example config plus scoped SHA256SUMS; native multi-architecture tagged workflow ownership and target-platform feasibility remain pending |
...
Prove the new GoReleaser Linux amd64/arm64 cross-CGO archive job in real Ubuntu CI and run the Helm upgrade/recreation/rollback smoke in an authenticated disposable cluster. Then extend descriptor parity to cloud/build variants, classify the remaining settings, and make migration-interruption acceptance a release prerequisite before more Cobra, filesystem, or `internal/` layout waves. Enable Homebrew tap and Scoop bucket publishing only after real Darwin/Windows CGO artifacts pass install/upgrade/uninstall lifecycle smokes.
