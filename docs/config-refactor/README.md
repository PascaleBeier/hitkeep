...
- `refactor/config-release-candidate` preserves the seven local refactor commits and the previously uncommitted configuration, recovery, and release changes in checkpoint `5d4124d9`. Merge `a4fee789` incorporates upstream `c9de71bb`, including offline DuckDB extensions and filter handling, without dropping the root-level package moves.
- Production flag registration now projects the canonical catalog. The legacy tag-based loader and registration helpers live in `config/legacy_loader_test.go`; the parity oracle remains intact until stabilization evidence permits contraction. This is not removal of the compatibility proof.
- Recovery resolves the explicit root configuration through `LoadArgs` instead of process-global arguments. Regression coverage includes all five recovery routes, both leading config spellings, and file/environment/recovery-flag precedence with an aborted operation that leaves the database absent.
- Release archives package the existing native binaries deterministically and verify their bytes, configuration manifest, and required member set. The upstream DuckDB extension licenses remain included. Rejection tests require the intended error rather than passing on a missing native fixture.
- The latest stable tag `v2.13.18` exists, but [release run 33489007153](https://github.com/PascaleBeier/hitkeep/actions/runs/33489007153) failed at promotion and skipped downstream docs publication. Tag existence does not close the transitive stabilization gate. A successful tagged release and its downstream evidence remain required before removing the oracle.
- Independent review found and corrected an unsupported `gh release view` field in promotion: published draft/tag state and REST latest-release identity are now verified separately, with an executable workflow regression. The adjacent private-docs parity check is optional when that checkout is absent; producer contract checks remain mandatory.
- Docs companion is published on docs `main` at `181497d`, including the v2 receiver, build-site action, artifact-only deployment, regression tests, recovery configuration guide, and structural SEO fixes. It corrects prepublication digest selection, absent-provenance JSON nulls, reusable-call detection, and deployment dependencies, and retains the existing full-site schedule. HitKeep pins its receiver SHA-256 `812c7896202be86362a5c384c17b8e87c461fac1353ec44210c79b15b4e58c08`, including revision-bound self-repository action references. The companion prerequisite is landed; the candidate still requires its own stable release. Unrelated dirty docs/sitemap work remains outside the companion. Do not repin v1 or bypass attestation to publish.
- The combined candidate requires source-bound full QA. Historical passing runs below are not evidence for this candidate. No new package-move or filesystem wave is included.
...
- Phase 10: the authorized move-only `internal/devtool/cli` → `devtool/cli` slice is complete. Its exact three-file inventory, sole runtime importer, inward dependencies, explicit production/cloud path guards, native terminal/summary/log boundaries, and command compatibility tests remain pinned; no compatibility shim was added.
- Phase 0C: `internal/devtool/devmcp` had an exact three-file retained record. After nested subprocess delegation was removed, the developer-only adapter moved to `devtool/devmcp` while retaining its direct dependency on shared `internal/devtool` workspace/run state.
- Phase 10: `internal/assetstore` → `assetstore` is an authorized two-file move-only wave. It preserves rooted QR asset mkdir, temporary creation, rename cleanup, open, removal, recursive removal, parent pruning, and symlink handling with `os.OpenRoot`/`os.Root` or `os.OpenInRoot`; no compatibility shim was added.
- Phase 10: `internal/cluster` → `cluster` is a completed two-file atomic move. `cmd/hitkeep.go` and `internal/server/server.go` now import the unchanged `Manager`, which remains behind `internal/server/shared.ClusterState`; deterministic join/leave reader transitions and concurrent event/read access are focused race proof. Constructor/bind parsing, `HasPeers`, and join-failure cleanup remain explicit memberlist lifecycle gaps; the intentionally empty `eventDelegate.NotifyUpdate` can leave a tracked leader-address mapping stale. No compatibility shim or live-network seam was added.
- Phase 10: `internal/realtime` → `realtime` is a completed three-file atomic move. Fourteen production and test importers now use the unchanged in-memory `Broker`, `Subscription`, and `Event` API. Existing race proof covers site isolation, replay/resync, slow subscribers, shutdown, and close versus subscribe/publish. Persistence, cross-leader replay, immediate mid-stream authorization revocation, and exact broader-consumer closure remain explicit gaps; no compatibility shim was added.
...
- Phase 10: `internal/reporting` → `reporting` is a completed three-file atomic move. Eight database, handler, worker, and test importers now use the unchanged scheduling and token API. Focused proof covers schedule validation, DST gaps/overlaps, local-time preservation, period bounds, 32-byte confirmation-token shape/hash behavior, HMAC-bound unsubscribe round-trip/tamper rejection, confirmation lifecycle, and recipient isolation. It remains application-internal despite the root import path; no compatibility shim was added.
