...
| 0C | In progress | Inventory filesystem operations and dependency-ordered `internal/` move manifest | The durable filesystem/package manifest records reviewed families, classifications, native exclusions, and rejected move candidates. `internal/assetstore` has rooted symlink-containment proof; `internal/blocking` now has bounded/closed feed-response, same-directory staged cache replacement (not fsync/durability or cross-platform atomicity proof), serialized whole-update generations, and asynchronous-cancelled refresh proof. Further Phase 9/10 waves remain frozen until every remaining direct child and operation has complete dependency/owner/build-tag/generated-file evidence |
