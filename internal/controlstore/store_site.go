...
func (s *Store) CreateSite(ctx context.Context, userID uuid.UUID, domain string) (*api.Site, error) {
	...
	_, err = tx.ExecContext(ctx,
		"INSERT INTO site_members (id, site_id, user_id, role, added_by) VALUES (?, ?, ?, 'owner', ?)",
		uuid.New(), id, userID, userID,
	)
	if err != nil {
...
// SetSiteCreatedAt updates the control-plane ordering timestamp for a site.
// It is intentionally bounded to this single field so callers never need raw
// SQL access to the SQLite control database.
func (s *Store) SetSiteCreatedAt(ctx context.Context, siteID uuid.UUID, createdAt time.Time) error {
	result, err := s.db.ExecContext(ctx, "UPDATE sites SET created_at = ? WHERE id = ?", createdAt.UTC(), siteID)
	if err != nil {
		return fmt.Errorf("update site creation timestamp: %w", err)
	}
...
func (s *Store) UpdateSiteDomain(ctx context.Context, siteID uuid.UUID, domain string) error {
	...
	if err := s.db.QueryRowContext(ctx, "SELECT domain FROM sites WHERE id = ?", siteID).Scan(&previousDomain); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
	...
	return s.transact(ctx, func(tx *sql.Tx) error {
		result, err := tx.ExecContext(ctx, "UPDATE sites SET domain = ? WHERE id = ?", domain, siteID)
		if err != nil {
			return fmt.Errorf("could not update site domain: %w", err)
		}
		affected, err := result.RowsAffected()
		if err != nil {
			return fmt.Errorf("could not inspect site domain update: %w", err)
		}
		if affected == 0 {
			return fmt.Errorf("site %s not found", siteID)
		}
...
func (s *Store) ListAllSites(ctx context.Context) ([]api.Site, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT s.id, s.user_id, s.domain, s.data_retention_days,
		        s.retention_synced_from_plan, s.created_at, COALESCE(u.email, '') AS owner_email
		 FROM sites s
	...
	for rows.Next() {
		var site api.Site
		if err := rows.Scan(&site.ID, &site.UserID, &site.Domain, &site.DataRetentionDays,
			&site.RetentionSyncedFromPlan, &site.CreatedAt, &site.OwnerEmail); err != nil {
			return nil, err
		}
		sites = append(sites, site)
	}
...
func (s *Store) DeleteSite(ctx context.Context, siteID uuid.UUID) error {
	...
	err := s.transact(ctx, func(tx *sql.Tx) error {
		...
		if _, err := tx.ExecContext(ctx, "DELETE FROM sites WHERE id = ?", siteID); err != nil {
			refs, refErr := findSiteReferences(ctx, tx, siteID)
			if refErr != nil {
...
// DeleteSiteWithWebhookEvent coordinates site cleanup with a durable staged
// outbox. The final deliveries cannot become dispatchable until the site
// deletion succeeds, while a recovery sweep can finish materializing them
