...
func (s *Store) ListInstanceAuditEntries(ctx context.Context, filter InstanceAuditFilter) ([]api.InstanceAuditEntry, int, error) {
	...
	if filter.Query != "" {
		q := "%" + filter.Query + "%"
		whereClauses = append(whereClauses, "(ial.actor_email_snapshot LIKE ? OR ial.target_label LIKE ? OR ial.details LIKE ? OR ial.ip_address LIKE ? OR ial.ip_country_code LIKE ? OR ial.request_id LIKE ?)")
...
// LatestInstanceAuditMetadataJSON returns the structured metadata for the most
// recent matching audit entry. It keeps callers on a bounded audit operation
// without exposing the SQLite pool.
func (s *Store) LatestInstanceAuditMetadataJSON(ctx context.Context, action string) (string, error) {
	var metadata string
	err := s.db.QueryRowContext(ctx, `
		...
		LIMIT 1
	`, action).Scan(&metadata)
	if err != nil {
		return "", fmt.Errorf("read latest instance audit metadata: %w", err)
	}
	return metadata, nil
}
