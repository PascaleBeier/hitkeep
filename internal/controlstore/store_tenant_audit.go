...
func (s *Store) ListTeamAuditEntriesFiltered(ctx context.Context, tenantID uuid.UUID, filter TeamAuditFilter) ([]api.TeamAuditEntry, int, error) {
	...
	if query := strings.TrimSpace(filter.Query); query != "" {
		q := "%" + query + "%"
		whereClauses = append(whereClauses, "(ta.actor_email_snapshot LIKE ? OR ta.target_label LIKE ? OR ta.details LIKE ? OR ta.ip_address LIKE ? OR ta.ip_country_code LIKE ? OR ta.request_id LIKE ?)")
...
func (s *Store) CreateTenant(ctx context.Context, creatorID uuid.UUID, name, logoURL string) (*api.Team, error) {
	...
	err := s.transact(ctx, func(tx *sql.Tx) error {
		...
		if _, err := tx.ExecContext(ctx,
			"INSERT INTO tenant_members (id, tenant_id, user_id, role, added_by) VALUES (?, ?, ?, ?, ?)",
			uuid.New(), tenantID, creatorID, TenantRoleOwner, creatorID,
		); err != nil {
