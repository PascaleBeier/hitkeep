...
func (s *Store) GetGoogleSearchConsoleSystemStatus(ctx context.Context) (GoogleSearchConsoleSystemStatus, error) {
	...
	var lastSuccessRaw, lastAttemptRaw, nextRetryRaw any
	...
	lastSuccess, err := parseSQLiteTime(lastSuccessRaw)
	if err != nil {
		return status, fmt.Errorf("parse Google Search Console last success: %w", err)
	}
	lastAttempt, err := parseSQLiteTime(lastAttemptRaw)
	if err != nil {
		return status, fmt.Errorf("parse Google Search Console last attempt: %w", err)
	}
	nextRetry, err := parseSQLiteTime(nextRetryRaw)
	if err != nil {
		return status, fmt.Errorf("parse Google Search Console next retry: %w", err)
	}
	status.LastSuccessAt = timePointer(lastSuccess)
	status.LastAttemptAt = timePointer(lastAttempt)
	status.NextRetryAt = timePointer(nextRetry)
	return status, nil
}
