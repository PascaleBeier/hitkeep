...
func (s *Store) EnsureDefaultTenant(ctx context.Context) (uuid.UUID, error) {
	...
	if _, err := tx.ExecContext(ctx, `
		INSERT OR IGNORE INTO data_migrations(name, applied_at) VALUES
			('default_tenant_split_v1', CURRENT_TIMESTAMP),
			('default_tenant_split_compacted_v1', CURRENT_TIMESTAMP)
	`); err != nil {
		return uuid.Nil, fmt.Errorf("mark fresh control plane split complete: %w", err)
	}
	id, err := getDefaultTenantID(ctx, tx)
	if err != nil {
		return uuid.Nil, err
	}
...
// LegacyControlImportComplete reports whether this SQLite control database was
// published from a post-split DuckDB source. Fresh SQLite installations do not
// carry this marker.
func (s *Store) LegacyControlImportComplete(ctx context.Context) (bool, error) {
	var count int
	err := s.db.QueryRowContext(ctx,
		"SELECT count(*) FROM control_imports WHERE name = ?",
		DuckDBControlImportV1,
	).Scan(&count)
	return count == 1, err
}
