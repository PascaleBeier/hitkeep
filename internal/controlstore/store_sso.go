...
// CountTeamSSOIdentities reports how many external identities are linked to a
// team without exposing the control database's raw query surface.
func (s *Store) CountTeamSSOIdentities(ctx context.Context, teamID uuid.UUID) (int, error) {
	var count int
	if err := s.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM sso_identities WHERE tenant_id = ?", teamID).Scan(&count); err != nil {
		return 0, fmt.Errorf("count team SSO identities: %w", err)
	}
	return count, nil
}
...
func (s *Store) DeleteTeamSSOConfig(ctx context.Context, teamID uuid.UUID) error {
	return s.transact(ctx, func(tx *sql.Tx) error {
		if _, err := tx.ExecContext(ctx, "DELETE FROM team_sso_domains WHERE tenant_id = ?", teamID); err != nil {
			return fmt.Errorf("could not delete team SSO domains: %w", err)
		}
		result, err := tx.ExecContext(ctx, "DELETE FROM team_sso_configs WHERE tenant_id = ?", teamID)
		if err != nil {
			return fmt.Errorf("could not delete team SSO configuration: %w", err)
		}
		affected, err := result.RowsAffected()
		if err != nil {
			return fmt.Errorf("could not inspect deleted team SSO configuration: %w", err)
		}
		if affected == 0 {
