...
func Run(ctx context.Context, args []string, stdout io.Writer) error {
	...
	if defaultReleaseRefresh {
		return runCompactReleaseRefresh(ctx, outPath, pkg, ip2locationToken)
	}
	...
	opts, closers, err := buildOptions(ctx, pkg, db1, db3, asn, db1v6, db3v6, asnv6)
	if err != nil {
		return err
	}
	...
	return writeGeneratedOutput(ctx, outPath, opts)
}

func runCompactReleaseRefresh(ctx context.Context, outPath string, pkg string, explicitToken string) error {
	...
	opts, closers, err := buildOptions(ctx, pkg, db1, nil, nil, db1v6, nil, nil)
	if err != nil {
		return err
	}
	defer closeAll(closers)
	...
	opts.CountryAssetDir = filepath.Dir(outPath)
	if err := writeGeneratedOutput(ctx, outPath, opts); err != nil {
		return err
	}

	cityBIN, err := downloadBINData(ctx, ip2locationDownloadURL(token, ip2locationDB3LiteBinIPv6Code))
	if err != nil {
		return err
	}
	if err := ctx.Err(); err != nil {
	...
	asnBIN, err := downloadBINData(ctx, ip2locationDownloadURL(token, ip2locationASNLiteBinIPv6Code))
	if err != nil {
		return err
	}
	if err := ctx.Err(); err != nil {
...
func buildOptions(ctx context.Context, pkg string, db1, db3, asn, db1v6, db3v6, asnv6 pathList) (ipmetagen.GenerateOptions, []io.Closer, error) {
	...
	for _, item := range []struct {
		...
		for _, path := range item.paths {
			...
			input, closer, err := openCSV(ctx, path, item.family)
			if err != nil {
				return ipmetagen.GenerateOptions{}, closers, err
			}
...
func openCSV(ctx context.Context, source string, family ipmetagen.AddressFamily) (ipmetagen.CSVInput, io.Closer, error) {
	if isURL(source) {
		return openRemoteCSV(ctx, source, family)
	}
	if isZIP(source) {
...
func writeGeneratedOutput(ctx context.Context, outPath string, opts ipmetagen.GenerateOptions) error {
	if err := ctx.Err(); err != nil {
		return err
	}
...
func downloadBINData(ctx context.Context, source string) ([]byte, error) {
	resp, err := getURL(ctx, source)
	if err != nil {
	...
	if len(body) < 64 {
		return nil, fmt.Errorf("download %s did not return a ZIP or BIN payload: %s", safeSourceName(source), responseSnippet(body, source))
	}
	return body, nil
}

func responseSnippet(body []byte, source string) string {
...
func openRemoteCSV(ctx context.Context, source string, family ipmetagen.AddressFamily) (ipmetagen.CSVInput, io.Closer, error) {
	resp, err := getURL(ctx, source)
	if err != nil {
...
func getURL(ctx context.Context, source string) (*http.Response, error) {
	...
	if err != nil {
		...
		if networkErr, ok := errors.AsType[net.Error](err); ok && networkErr.Timeout() {
			return nil, errors.New("network request timed out")
		}
