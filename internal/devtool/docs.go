...
func ValidateDevelopmentDocs(root string) error {
	files := map[string][]string{
		...
		"CONTRIBUTING.md":              {"./hk help", "./hk catalog --output json", "./hk catalog commands --output json", "agent_command", "model-agnostic", "central MCP", "configured or enabled", "explicit user approval", "server-catalog routing", "stateless protocol mode", "--fallback-workspace", "macOS or Linux", "AMD64 or ARM64", "executable POSIX launcher", "./hk mcp manifest", "./hk mcp serve", "./hk skills check", "./hk screenshot", "hk_screenshot", "./hk fmt", "./hk fmt --scope frontend", "./hk fix check", "./hk cache status", "./hk run list", "hk.dev/v2", "hk_dev_status", "next_cursor", "AGENTS.md", "source repository is private"},
