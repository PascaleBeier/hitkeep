...
func classifyChangedPath(path string) ([]string, bool) {
	...
	case strings.HasPrefix(path, ".github/") || strings.HasPrefix(path, "charts/") || path == ".goreleaser.yaml" || path == "Dockerfile" || path == "scripts/docker-smoke.sh" || strings.HasPrefix(path, "compose"):
