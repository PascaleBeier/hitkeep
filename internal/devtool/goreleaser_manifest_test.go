...
func TestFilesystemLayoutManifestPinsReviewedFamilyRecords(t *testing.T) {
	...
	records := map[string][]string{
		...
		"### `internal/ai`":                                     {"bounded direct importers", "exhaustive transitive closure is not claimed", "Config.Timeout", "RunRecorder", "AppendAIRun", "ReserveAIRun", "validated", "no raw provider", "TestLiveOpenAIOpportunityCandidateProposalSmoke", "no compatibility shim", "decomposition required / stay internal / blocked"},
		...
		"### `internal/server`":                                 {"`internal/server` → `internal/server`; no move is approved", "Required decomposition", "Gortex returned 50 files", "bounded result", "bounded direct-importer inventory is unavailable", "heterogeneous imports", "complete cycle proof is unavailable", "concrete focused subpackage targets", "affected-consumer closure", "direct rollback", "no compatibility shim", "inbound HTTP", "database/store", "network-facing integrations", "billing/OSS variants", "Unix-specific disk-usage", "Complete build-tag, OS, CGO, generated, and embed closure is not established", "exact test closure is incomplete", "No compatibility shim or forwarding package is justified", "decomposition required / stay internal / blocked"},
