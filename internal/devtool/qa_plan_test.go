...
func TestReleaseFilesSelectDeliveryArea(t *testing.T) {
	for _, path := range []string{"scripts/docker-smoke.sh", "scripts/compose-smoke.sh", "scripts/update-default-ai-agents.sh", "scripts/update-default-spam-filter.sh", ".goreleaser.yaml"} {
