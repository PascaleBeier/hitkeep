...
var gates = []Gate{
	...
	{ID: "self-hosted-image", Description: "Build, recreate, and persistence-smoke the self-hosted production image", Command: []string{"docker", "buildx", "build", ".", "--target", "local-image", "--load"}, Profiles: []string{"full"}, Paths: []string{"Dockerfile", ".dockerignore", "cmd/", "internal/", "frontend/", "scripts/docker-smoke.sh"}, Weight: 4, Timeout: "35m"},
