...
var gates = []Gate{
	...
	{ID: "go-lint", Description: "Lint Go code with the pinned CI configuration", CIGroup: "golangci", Command: []string{"go", "run", "github.com/golangci/golangci-lint/v2/cmd/golangci-lint@v" + "2.13.0", "run"}, Profiles: []string{"pr", "full"}, Paths: []string{"*.go", "go.mod", "go.sum", ".golangci.yml"}, Weight: 2, Timeout: "10m"},
	...
	{ID: "go-staticcheck", Description: "Run pinned Staticcheck", CIGroup: "go-checks", Command: []string{"go", "run", "honnef.co/go/tools/cmd/staticcheck@v" + "0.8.0-rc.1", "./..."}, Profiles: []string{"pr", "full"}, Paths: []string{"*.go", "go.mod", "go.sum"}, Weight: 2, Timeout: "15m"},
	...
	{ID: "developer-mcp", Description: "Validate the developer CLI/MCP contract and production binary boundary", CIGroup: "go-checks", Command: []string{"go", "test", "./cmd/hk", "./devtool/devmcp"}, Profiles: []string{"pr", "full"}, Paths: []string{"cmd/hk/", "cmd/hitkeep/", "devtool/", "internal/devtool/", "hk", "go.mod", "go.sum"}, Weight: 1, Timeout: "5m"},
	...
	{ID: "govulncheck", Description: "Scan Go dependencies for reachable vulnerabilities", CIGroup: "govulncheck", Command: []string{"go", "run", "golang.org/x/vuln/cmd/govulncheck@v" + "1.6.0", "./..."}, Profiles: []string{"pr", "full"}, Paths: []string{"*.go", "go.mod", "go.sum"}, Weight: 2, Timeout: "15m"},
